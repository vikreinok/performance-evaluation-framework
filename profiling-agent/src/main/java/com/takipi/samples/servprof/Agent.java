...
public class Agent {
	...
	private static Transformer internalPremain(String agentArgs, Instrumentation inst) throws FileNotFoundException {
		...
		InstrumentationFilter instFilter = new InstrumentationFilter(options.getClassPattern(), options.getMethodPattern());
		...
		BaseReporter reporter = ReporterFactory.createStatsdReporter("profile-reporter", options.getReportIntervalMillis());
